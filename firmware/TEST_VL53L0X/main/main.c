...
static const char *TAG = "Robot_Dual_Task";
...
// ✅ Mảng lưu giá trị mới nhất
static uint16_t sensor_distances[NUM_SENSORS] = {0};
static uint8_t  sensor_status[NUM_SENSORS]    = {255}; // 255 = chưa có data
...
// --- Task ISR-driven ---
void vl53l0x_sensor_task(void *pvParameters) {
...
// --- Chuỗi khởi tạo cảm biến ---
void init_sensors_sequence(void) {
...
void app_main(void) {
    i2c_bus_init();
    init_sensors_sequence();

    // ✅ Task in bảng tổng hợp
    xTaskCreate(print_summary_task, "print_summary", 2048, NULL, 3, NULL);
